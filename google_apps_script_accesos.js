...
var destinatariosGlobal = "alfredo.netro@deppg.com, alba.garcia@deppg.com, gilberto.martell@deppg.com";
...
function registrarAcceso(data) {
  ...
  var fecha = Utilities.formatDate(new Date(ts), "GMT-6", "yyyy-MM-dd HH:mm:ss");
...
function doPost(e) {
  try {
    ...
    var ahora = new Date();
    var timestampStr = Utilities.formatDate(ahora, "GMT-6", "yyyy-MM-dd HH:mm:ss");
    
    var nombreEnviado = (data.p) ? data.p.toString().trim() : "DESCONOCIDO"; 

    // --- FILTRO DE SEGURIDAD ---
    if (POZOS_IGNORADOS.indexOf(nombreEnviado) !== -1) {
      return ContentService.createTextOutput("IGNORADO").setMimeType(ContentService.MimeType.TEXT);
    }
    ...
    function registrarEvento(nombreFinal, datosVar, presion) {
      ...
      console.log("Pozo: " + nombreFinal + " | Estado Anterior: " + estadoPrevio + " | Alarma Actual: " + esAlarmaActual);
      ...
      if (esAlarmaActual && estadoPrevio !== "Alarma") {
        ...
        enviarEmail(nombreFinal, "SISTEMA RESTABLECIDO", false, datosVar, presion);
        props.setProperty("status_" + nombreFinal, "Normal");
      }

      var fila = [
        timestampStr, nombreFinal, tipoRegistro, 
        esAlarmaActual ? "" : datosVar[3], esAlarmaActual ? "" : datosVar[2], 
        esAlarmaActual ? "" : datosVar[0], esAlarmaActual ? "" : datosVar[4], 
        esAlarmaActual ? "" : g[0], esAlarmaActual ? "" : g[2], 
        esAlarmaActual ? "" : g[1], esAlarmaActual ? "" : g[3], 
        esAlarmaActual ? "" : presion, (esAlarmaActual ? falla : "Normal")
      ];

      sheet.appendRow(fila);
      if (!esAlarmaActual) sheet.getRange(sheet.getLastRow(), 4, 1, 9).setNumberFormat("0.0");
    }
...
function enviarEmail(pozo, evento, esAlarma, v, p) {
  ...
  var cuerpo = "TELEMETRÍA ÉBANO - REPORTE\n" +
               "-------------------------------------\n" +
               "Pozo: " + pozo + "\n" +
               "Evento: " + evento + "\n" +
               "Presión: " + p + " psi\n" +
...
function resetearEstados() {
  ...
  console.log("Memoria de estados limpiada. El siguiente reporte enviará correo sí o sí.");
}
